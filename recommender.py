...
def load_model():
    """
    Heavy lifting: load CSVs, build similarity matrices.
    Call this once from app.py inside @st.cache_resource.
    Returns (movies_df, new_df, content_sim, collab_sim_or_None)
    """
    movies  = pd.read_csv("data/tmdb_5000_movies.csv")
    credits = pd.read_csv("data/tmdb_5000_credits.csv")
    movies  = movies.merge(credits, on="title")
    ...
    if os.path.exists(RATINGS_PATH) and os.path.exists(LINKS_PATH):
        ...
        if not ratings.empty:
            ...
            # Vectorized scatter — replaces O(n²) Python loop
            n         = len(new_df)
            collab_sim = np.zeros((n, n))
            id_to_idx  = {mid: idx for idx, mid in enumerate(new_df["movie_id"].tolist())}

            row_idx = np.array([id_to_idx[mid] for mid in aligned_ids])
            # Place collab_sim_partial rows/cols into the full n×n matrix in one shot
            collab_sim[np.ix_(row_idx, row_idx)] = collab_sim_partial
    ...
    return movies, new_df, content_sim, collab_sim


def recommend(movie_name, movies, new_df, content_sim, collab_sim, content_weight=0.6, collab_weight=0.4):
    matches = new_df[new_df["title"].str.contains(movie_name, case=False, na=False, regex=False)]
    if matches.empty:
        return ["Movie not found"]
